...
2. 确认目标数据库路径。默认桌面版用户数据路径通常是：  
   ...
   ```bash
   ~/Library/Application\ Support/SuiXinAI/lobsterai.sqlite
   ```
...
npm run test:sqlite-backup:seed -- \
  --db '/Users/jj.deng/Library/Application Support/SuiXinAI/lobsterai.sqlite' \
  --sessions 10 \
...
```bash
ls -lh ~/Library/Application\ Support/SuiXinAI/backups/sqlite/snapshots/
```
...
```bash
printf 'not-a-sqlite-db' > ~/Library/Application\ Support/SuiXinAI/lobsterai.sqlite
```
