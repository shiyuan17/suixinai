...
test('guards new automation-task wording when no cron.add succeeded', () => {
  const analysis = analyzeIMReply([
    {
      id: 'assistant-1',
      type: 'assistant',
      content: '自动化任务创建成功！到时间后我会自动提醒你。',
      timestamp: Date.now(),
      metadata: {},
    },
  ]);
...
test('recognizes English automation-task wording for compatibility', () => {
  const analysis = analyzeIMReply([
    {
      id: 'assistant-1',
      type: 'assistant',
      content: 'Automation task created successfully. I will remind you when it is time.',
      timestamp: Date.now(),
      metadata: {},
    },
  ]);
...
test('uses the new automation-task fallback after successful cron.add with no assistant text', () => {
  const analysis = analyzeIMReply([
    {
      id: 'tool-use-1',
      type: 'tool_use',
  ...
  assert.equal(analysis.guardApplied, false);
  assert.equal(analysis.successfulCronAdds, 1);
  assert.equal(analysis.text, '已创建自动化任务。');
});
