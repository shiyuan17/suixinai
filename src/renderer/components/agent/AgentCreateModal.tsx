...
const AgentCreateModal: React.FC<AgentCreateModalProps> = ({
  ...
  const [presetDivisionFilter, setPresetDivisionFilter] = useState<string>(ALL_PRESET_DIVISION_FILTER);
  const [activeTab, setActiveTab] = useState<AgentDetailTab>(AgentDetailTab.Market);
  ...
  useEffect(() => {
    ...
    setSkillIds([]);
    setActiveTab(AgentDetailTab.Market);
    setPresetSearchQuery('');
    setPresetDivisionFilter(ALL_PRESET_DIVISION_FILTER);
  ...
  const tabs: { key: AgentDetailTab; label: string }[] = [
    { key: AgentDetailTab.Market, label: i18nService.t('agentMarketTitle') },
  ...
  const editorContent = (
    <>
      ...
      <div className="px-7 py-7 overflow-hidden flex-1 min-h-0">
        {activeTab === AgentDetailTab.Market && (
          <AgentTemplatePickerContent
            presets={presetTemplates}
            loading={templatesLoading}
            searchQuery={presetSearchQuery}
            onSearchQueryChange={setPresetSearchQuery}
            selectedDivision={presetDivisionFilter}
            onDivisionChange={setPresetDivisionFilter}
            onSelect={handleApplyTemplate}
            onAdd={agentService.addPreset}
          />
        )}
...
export const AgentTemplatePickerContent: React.FC<{
  presets: PresetAgent[];
  loading: boolean;
  searchQuery: string;
  ...
  onAdd: (presetId: string) => Promise<unknown>;
}> = ({
  presets,
  loading,
  searchQuery,
  ...
  const divisionOptions = buildPresetDivisionFilterOptions(
    presets,
    isEn,
    i18nService.t('agentMarketDivisionAll'),
  );
  const visiblePresets = filterPresetMarket(presets, searchQuery, selectedDivision, isEn);

  const handleAdd = async (presetId: string) => {
    setAddingId(presetId);
    try {
      await onAdd(presetId);
    } finally {
      setAddingId(null);
    }
  };
    ...
    <div className="h-full min-h-0 overflow-y-auto">
      <div className="mb-4">
        <input
          ...
          placeholder={i18nService.t('agentMarketSearchPlaceholder')}
          aria-label={i18nService.t('agentMarketSearchPlaceholder')}
          className="w-full rounded-xl border border-border bg-surface px-3 py-2 text-sm text-foreground placeholder:text-secondary/60 focus:border-primary focus:outline-none"
        />
      </div>
      <div
        role="tablist"
        aria-label={i18nService.t('agentMarketDivisionAll')}
      ...
      >
          {divisionOptions.map(option => (
            <button
              ...
              type="button"
              role="tab"
              aria-selected={selectedDivision === option.value}
              onClick={() => onDivisionChange(option.value)}
              className={`rounded-full border px-3 py-1.5 text-xs font-medium transition-colors ${
                selectedDivision === option.value
                  ? 'border-primary bg-primary/10 text-primary'
                  : 'border-border bg-surface text-secondary hover:border-primary/30 hover:bg-surface-raised hover:text-foreground'
              }`}
            >
              {option.label}
            </button>
      ...
      <div className="mb-4 text-xs text-secondary">
        {i18nService.t('agentMarketResultsCount').replace('{count}', String(visiblePresets.length))}
      </div>
      {loading ? (
        ...
        <div className="flex h-full min-h-[160px] items-center justify-center text-sm text-secondary">
          {i18nService.t('agentMarketEmpty')}
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          {visiblePresets.map((preset) => {
            ...
            const divisionLabel = getPresetDivisionLabel(preset, isEn);
            const isInstalled = preset.installed === true;
            const isAdding = addingId === preset.id;

            return (
              <div
                key={preset.id}
                className={`group flex min-h-[164px] flex-col rounded-xl border bg-surface p-4 text-left transition-colors ${
                  isInstalled ? 'border-border/80 opacity-75' : 'border-border hover:border-primary/40 hover:bg-surface-raised'
                }`}
              >
                ...
                <div className="mt-4 flex items-center justify-end gap-2">
                  {!isInstalled && (
                    <button
                      ...
                      className="px-3 py-1.5 text-xs font-medium text-secondary transition-colors hover:text-foreground"
                    >
                  ...
                  <button
                    ...
                    className={`rounded-lg px-3 py-1.5 text-xs font-medium transition-colors ${
                      isInstalled
                        ? 'cursor-not-allowed bg-surface-raised text-secondary'
                        : 'bg-primary text-white hover:bg-primary-hover disabled:opacity-60'
                    }`}
                  >
                    {isInstalled
                      ? i18nService.t('agentMarketInstalled')
                      : isAdding
                        ? i18nService.t('creating')
                        : i18nService.t('addAgent')}
                  </button>
                </div>
              </div>
            );
