...
import { reportScheduledTaskAction } from '../scheduledTasks/analytics';
...
const CoworkView: React.FC<CoworkViewProps> = ({
  ...
  const homeHeader = (
    <div className="draggable flex h-14 items-center justify-between px-4 shrink-0">
      ...
      <div className="non-draggable flex items-center">
        <button
          type="button"
          onClick={() => {
            reportScheduledTaskAction('open_scheduled_tasks', {
              source: 'cowork_home_header',
            });
            onShowScheduledTasks?.();
          }}
          className="h-8 w-8 inline-flex items-center justify-center rounded-lg text-secondary hover:bg-surface-raised transition-colors mr-2"
          aria-label={i18nService.t('scheduledTasksTitle')}
          title={i18nService.t('scheduledTasksTitle')}
        >
          <SidebarAutomationIcon className="h-5 w-5" />
        </button>
        <WindowTitleBar inline />
      </div>
    </div>
  ...
  return (
    <div className="flex-1 flex flex-col bg-background h-full">
      ...
      <div className="flex-1 overflow-y-auto min-h-0 relative">
        <div className="relative mx-auto flex min-h-full w-full min-w-[320px] max-w-[1120px] flex-col items-center px-4 pb-10 pt-[clamp(56px,13vh,108px)]">
          <div className="flex w-full flex-col items-center">
            <div className="w-full max-w-[760px] text-center">
              <p
                className="text-[26px] font-semibold leading-8 text-foreground animate-fade-in-up"
                style={{ animationDelay: '35ms', animationFillMode: 'both' }}
              >
                {i18nService.t('coworkWelcome')}
              </p>
              <h1
                className="mt-2 whitespace-nowrap text-[34px] font-semibold leading-[1.12] text-foreground animate-fade-in-up sm:text-[40px] lg:text-[44px]"
                style={{ animationDelay: '85ms', animationFillMode: 'both' }}
              >
                {i18nService.t('coworkDescription')}{i18nService.t('coworkHeroHeadline')}
              </h1>
            </div>

            <div
              className="relative z-20 mt-9 flex w-full max-w-[760px] flex-col items-center"
              ...
              >
                <div className="flex w-full flex-col items-center">
                  <div className="mb-4 flex w-full justify-center">
                    {selectedAction ? (
                      <div className="w-full rounded-xl border border-border bg-surface px-4 py-4 sm:px-5">
                        <PromptPanel
