...
import { authService, isAuthLoginCancelledError } from './services/auth';
...
const SETTINGS_TAB_SHORTCUT_ACTIONS: Array<{
  ...
  { action: ShortcutAction.OpenMcp, initialTab: 'mcp' },
...
const App: React.FC = () => {
  ...
  const [mainView, setMainView] = useState<'cowork' | 'skills' | 'scheduledTasks' | 'kits'>('cowork');
  ...
  const [emailLoginDialogState, setEmailLoginDialogState] = useState(
    authService.getEmailLoginDialogState(),
  );
  ...
  const handleWelcomeLogin = useCallback(async () => {
    ...
    try {
      ...
      if (!isAuthLoginCancelledError(error)) {
        throw error;
      }
    }
  }, []);
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (showSettings) {
        if (matchesAction(ShortcutAction.OpenMcp)) {
          ...
          handleShowSettings({ initialTab: getSettingsTabForGlobalShortcut(ShortcutAction.OpenMcp) ?? 'mcp' });
        }
  ...
  useEffect(() => {
    return authService.subscribeEmailLoginDialog(setEmailLoginDialogState);
  }, []);
  ...
  return (
    <div className="h-screen overflow-hidden flex flex-col bg-surface-raised">
      ...
      <EmailLoginModal
        isOpen={emailLoginDialogState.isOpen}
        onClose={() => authService.closeEmailLoginDialog()}
      />
    </div>
  );
};
