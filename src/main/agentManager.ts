...
import { ALL_PRESET_AGENTS, PRESET_AGENTS, type PresetAgent, presetToCreateRequest } from './presetAgents';
...
export class AgentManager {
  ...
  getAllPresetAgents(): PresetAgent[] {
    const existingPresetIds = new Set(
      this.store.listAgents().filter(a => a.source === 'preset').map(a => a.presetId),
    );
    return ALL_PRESET_AGENTS.map(preset => ({
      ...preset,
      installed: existingPresetIds.has(preset.id),
    }));
  }
