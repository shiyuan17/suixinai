...
import { AuthIpcChannel, AuthMode } from '../shared/auth/constants';
import {
...
import {
  ...
  type CompleteAuthSessionFailureResult,
} from './libs/authSession';
import {
  createLocalDevProfileSummary,
  createLocalDevQuota,
  ensureLocalSupabaseSession,
  isLocalSessionExpiringSoon,
  LocalAuthStoreKey,
  LocalEmailCodeFailureReason,
  LocalEmailCodeStore,
  refreshLocalSupabaseSession,
...
const LOCAL_AUTH_FREE_PLAN_NAME = '免费';
const localEmailCodeStore = new LocalEmailCodeStore();
let cachedResolvedAuthConfig: ReturnType<typeof resolveLocalAuthConfig> | null = null;
...
if (!gotTheLock) {
  ...
  const getResolvedAuthConfig = () => {
    if (!cachedResolvedAuthConfig) {
      cachedResolvedAuthConfig = resolveLocalAuthConfig(app.getAppPath());
    }
    return cachedResolvedAuthConfig;
  };

  const getLocalAuthSession = (): LocalAuthSession | null => {
    try {
      return getStore().get<LocalAuthSession>(LocalAuthStoreKey.Session) || null;
    } catch (error) {
      console.warn('[Auth] failed to read local auth session:', error);
      return null;
    }
  };

  const saveLocalAuthSession = (session: LocalAuthSession) => {
    getStore().set(LocalAuthStoreKey.Session, session);
    getStore().set(LocalAuthStoreKey.User, session.user);
    saveAuthUser(session.user);
  };

  const clearLocalAuthSession = () => {
    try {
      getStore().delete(LocalAuthStoreKey.Session);
      getStore().delete(LocalAuthStoreKey.User);
    } catch (error) {
      console.warn('[Auth] failed to clear local auth session:', error);
    }
  };

  const getActiveAuthMode = (): typeof AuthMode.LocalSupabase | typeof AuthMode.Remote => {
    const resolvedConfig = getResolvedAuthConfig();
    if (resolvedConfig.mode === AuthMode.LocalSupabase) {
      return AuthMode.LocalSupabase;
    }
    return AuthMode.Remote;
  };

  const getLocalDevQuota = () => createLocalDevQuota(LOCAL_AUTH_FREE_PLAN_NAME);

  const ensureCurrentLocalAuthSession = async (reason: string): Promise<LocalAuthSession | null> => {
    const resolvedConfig = getResolvedAuthConfig();
    if (resolvedConfig.mode !== AuthMode.LocalSupabase) {
      return null;
    }
    ...
    if (!isLocalSessionExpiringSoon(session)) {
      return session;
    }

    try {
      const refreshedSession = await refreshLocalSupabaseSession(
        resolvedConfig.supabase,
        session,
        (input, init) => net.fetch(input, init),
      );
      saveLocalAuthSession(refreshedSession);
      console.log(`[Auth] refreshed local Supabase session (reason: ${reason})`);
      return refreshedSession;
    } catch (error) {
      console.warn(`[Auth] failed to refresh local Supabase session (reason: ${reason}):`, error);
  ...
  const refreshOnce = async (reason: string): Promise<string | null> => {
    ...
    pendingTokenRefresh = (async () => {
      try {
        if (getActiveAuthMode() === AuthMode.LocalSupabase) {
          const localSession = await ensureCurrentLocalAuthSession(reason);
          resolvedToken = localSession?.accessToken || null;
  ...
  ipcMain.handle(AuthIpcChannel.Exchange, async (_event, { code }: { code: string }) => {
    try {
      ...
      const body = (await resp.json()) as AuthSessionResponseBody;
      const result = completeAuthSession({
        body,
        getPreviousQuotaGateState: getAuthQuotaGateState,
        normalizeQuota,
        saveAuthTokens,
        saveAuthUser,
      ...
      if (!result.success) {
        const { error } = result as CompleteAuthSessionFailureResult;
        return { success: false, error };
      }
      clearLocalAuthSession();
      console.log('[Auth] exchange user data:', JSON.stringify(result.user));
      return { success: true, user: result.user, quota: result.quota };
    } catch (error) {
  ...
  ipcMain.handle(AuthIpcChannel.RequestEmailCode, async (_event, { email }: { email: string }) => {
    try {
      const resolvedConfig = getResolvedAuthConfig();
      if (resolvedConfig.mode === AuthMode.LocalSupabase) {
        const entry = localEmailCodeStore.issue(email);
        await sendLocalEmailCode({
          config: resolvedConfig.smtp,
          email,
          code: entry.code,
        });
        console.log(`[Auth] sent local email verification code to ${email.trim().toLowerCase()}`);
        return { success: true };
      }

      const serverBaseUrl = getServerApiBaseUrl();
      const requestEmailCodeUrl = `${serverBaseUrl}/api/auth/email/request-code`;
      console.log(`[Auth] requesting email verification code at ${requestEmailCodeUrl}`);
      const resp = await net.fetch(requestEmailCodeUrl, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(withKeyfromBody({ email })),
      });
      const body = (await resp.json()) as { code: number; message?: string };
      if (!resp.ok || body.code !== 0) {
        return { success: false, error: body.message || `Request email code failed: ${resp.status}` };
      }
      return { success: true };
    } catch (error) {
      console.error('[Auth] request email verification code failed:', error);
      return {
        success: false,
  ...
  ipcMain.handle(
    AuthIpcChannel.VerifyEmailCode,
    async (_event, { email, code }: { email: string; code: string }) => {
      try {
        const resolvedConfig = getResolvedAuthConfig();
        if (resolvedConfig.mode === AuthMode.LocalSupabase) {
          const verification = localEmailCodeStore.verify(email, code);
          if (verification.success === false) {
            if (verification.reason === LocalEmailCodeFailureReason.Missing) {
              return { success: false, error: '请先发送验证码' };
            }
            if (verification.reason === LocalEmailCodeFailureReason.Expired) {
              return { success: false, error: '验证码已过期，请重新发送' };
            }
            return { success: false, error: '验证码错误' };
          }

          const session = await ensureLocalSupabaseSession(
            resolvedConfig.supabase,
            email,
            (input, init) => net.fetch(input, init),
          );
          ...
          if (!quotaGateSyncScheduled) {
            syncOpenClawConfig({
              reason: 'local-auth-server-models-cleared',
              restartGatewayIfRunning: false,
            }).catch((error) => {
              console.warn('[Auth] failed to sync OpenClaw config after local login:', error);
            });
          }

          console.log(`[Auth] completed local Supabase login for ${session.email}`);
          return {
            success: true,
            user: session.user,
            quota: getLocalDevQuota(),
            authMode: AuthMode.LocalSupabase,
          };
        }

        const serverBaseUrl = getServerApiBaseUrl();
        const verifyEmailCodeUrl = `${serverBaseUrl}/api/auth/email/verify-code`;
        console.log(`[Auth] verifying email code at ${verifyEmailCodeUrl}`);
        const resp = await net.fetch(verifyEmailCodeUrl, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(withKeyfromBody({ email, code })),
        });
        const body = (await resp.json()) as AuthSessionResponseBody;
        const result = completeAuthSession({
          body,
        ...
        if (!resp.ok) {
          return {
            success: false,
            error: `Verify email code failed: ${resp.status}`,
          };
        }
        clearLocalAuthSession();
        console.log('[Auth] email verification user data:', JSON.stringify(result.user));
        return {
          success: true,
          user: result.user,
          quota: result.quota,
        ...
        console.error('[Auth] verify email code failed:', error);
        return {
          success: false,
  ...
  ipcMain.handle(AuthIpcChannel.GetUser, async () => {
    try {
      if (getActiveAuthMode() === AuthMode.LocalSupabase) {
        const session = await ensureCurrentLocalAuthSession('get-user');
        if (!session) {
      ...
      return { success: true, user: profileBody.data, quota, authMode: AuthMode.Remote };
    } catch {
      return { success: false };
    }
  });
  ...
  ipcMain.handle(AuthIpcChannel.GetQuota, async () => {
    try {
      if (getActiveAuthMode() === AuthMode.LocalSupabase) {
        const session = await ensureCurrentLocalAuthSession('get-quota');
        if (!session) {
          return { success: false };
        }
        return { success: true, quota: getLocalDevQuota(), authMode: AuthMode.LocalSupabase };
      }
      ...
      syncOpenClawConfigIfAuthQuotaGateChanged(previousQuotaGateState);
      return { success: true, quota, authMode: AuthMode.Remote };
    } catch {
      return { success: false };
    }
  });
  ...
  ipcMain.handle(AuthIpcChannel.GetProfileSummary, async () => {
    try {
      if (getActiveAuthMode() === AuthMode.LocalSupabase) {
        const session = await ensureCurrentLocalAuthSession('get-profile-summary');
        if (!session) {
          return { success: false };
        }
        return {
          success: true,
          data: createLocalDevProfileSummary(session.user),
          authMode: AuthMode.LocalSupabase,
        };
      }
      ...
      const body = (await resp.json()) as { code: number; data: Record<string, unknown> };
      if (body.code !== 0 || !body.data) return { success: false };
      return { success: true, data: body.data, authMode: AuthMode.Remote };
    } catch {
      return { success: false };
    }
  });
  ...
  ipcMain.handle(AuthIpcChannel.Logout, async () => {
    try {
      if (getActiveAuthMode() === AuthMode.LocalSupabase) {
        ...
        if (resolvedConfig.mode === AuthMode.LocalSupabase && localSession) {
          await net.fetch(`${resolvedConfig.supabase.url.replace(/\/$/, '')}/auth/v1/logout`, {
            method: 'POST',
            headers: {
              apikey: resolvedConfig.supabase.anonKey,
        ...
        if (!quotaGateSyncScheduled) {
          syncOpenClawConfig({
            ...
            console.warn('[Auth] failed to sync OpenClaw config after local logout:', error);
          });
        }
        console.log('[Auth] cleared local Supabase login state');
        return { success: true };
      }
  ...
  ipcMain.handle(AuthIpcChannel.RefreshToken, async () => {
    try {
      ...
      return accessToken
        ? { success: true, accessToken, authMode: getActiveAuthMode() }
        : { success: false };
    } catch {
      return { success: false };
    }
  ...
  ipcMain.handle(AuthIpcChannel.GetAccessToken, async () => {
    if (getActiveAuthMode() === AuthMode.LocalSupabase) {
      const session = await ensureCurrentLocalAuthSession('get-access-token');
  ...
  ipcMain.handle(AuthIpcChannel.GetModels, async () => {
    try {
      if (getActiveAuthMode() === AuthMode.LocalSupabase) {
        const session = await ensureCurrentLocalAuthSession('get-models');
        if (!session) {
          return { success: false };
        }
        clearServerModelMetadata();
        syncOpenClawConfig({
          ...
          console.warn('[Auth:getModels] failed to sync OpenClaw config for local auth mode:', error);
        });
        return {
          success: true,
          models: [],
      ...
      return { success: true, models: data.data, authMode: AuthMode.Remote };
    } catch (e) {
  ...
  ipcMain.handle(DataMigrationIpc.Backup, async event => {
    ...
    try {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to back up 随心 AI data',
      };
    }
  });
  ...
  ipcMain.handle(DataMigrationIpc.Restore, async event => {
    ...
    try {
      ...
      return {
        ...
        error: success ? undefined : restoreResult?.error || 'Failed to import 随心 AI data backup',
      };
    } catch (error) {
      isCleanupInProgress = false;
      const message = error instanceof Error ? error.message : 'Failed to import 随心 AI data backup';
