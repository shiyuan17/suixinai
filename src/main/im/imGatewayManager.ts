...
export class IMGatewayManager extends EventEmitter {
  ...
  constructor(db: Database.Database, options?: IMGatewayManagerOptions) {
    ...
    this.whatsappAdapter = new WhatsAppAdapter({
      ...
      processMessage: async (message) => this.processManagedMessage(message),
    });
  ...
  private setupMessageHandlers(): void {
    const messageHandler = async (
      ...
      try {
        const response = await this.processManagedMessage(message);
  ...
  private async processManagedMessage(message: IMMessage): Promise<string> {
    ...
    if (this.coworkHandler) {
      ...
      return this.coworkHandler.processMessage(message);
    }
    ...
    return this.chatHandler.processMessage(message);
  }
  ...
  setConfig(
    ...
    if (config.whatsapp) {
      const nextWhatsAppConfig = this.imStore.getWhatsAppConfig();
      void this.whatsappAdapter.sync(nextWhatsAppConfig).catch((error) => {
        console.error('[IMGatewayManager] Failed to sync WhatsApp adapter config:', error);
      });
    }
  ...
  getStatus(): IMGatewayStatus {
    ...
    return {
      ...
      whatsapp: this.whatsappAdapter.getStatus(),
      nim: {
  ...
  async startGateway(platform: Platform): Promise<void> {
    ...
    if (platform === 'dingtalk') {
      ...
      console.log('[IMGatewayManager] Starting WhatsApp adapter');
      await this.whatsappAdapter.sync(this.getConfig().whatsapp);
      return;
  ...
  async startAllEnabled(): Promise<void> {
    ...
    if (config.whatsapp?.enabled) {
      try {
        await this.whatsappAdapter.sync(config.whatsapp);
      } catch (error: any) {
        console.error(`[IMGatewayManager] Failed to start WhatsApp adapter: ${error.message}`);
      }
    }
  ...
  isConnected(platform: Platform): boolean {
    ...
    if (platform === 'whatsapp') {
      return this.whatsappAdapter.getStatus().connected;
    }
    if (platform === 'nim') {
  ...
  private buildMergedConfig(configOverride?: Partial<IMGatewayConfig>): IMGatewayConfig {
    ...
    return {
      ...
      whatsapp: { ...current.whatsapp, ...(configOverride.whatsapp || {}) },
  ...
  private getMissingCredentials(platform: Platform, config: IMGatewayConfig): string[] {
    ...
    if (platform === 'whatsapp') {
      const fields: string[] = [];
      if (!config.whatsapp?.webhookSecret) fields.push('webhookSecret');
      if (!config.whatsapp?.listenerBaseUrl) fields.push('listenerBaseUrl');
      return fields;
    }
    if (platform === 'popo') {
      const popoInsts = config.popo?.instances || [];
  ...
  private async runAuthProbe(platform: Platform, config: IMGatewayConfig): Promise<string> {
    ...
    if (platform === 'whatsapp') {
      return 'WhatsApp adapter configuration is ready.';
    }
  ...
  async sendConversationReply(platform: Platform, conversationId: string, text: string): Promise<boolean> {
    try {
      switch (platform) {
        case 'whatsapp':
          return this.whatsappAdapter.sendOutboundMessage(conversationId, text);
        default:
