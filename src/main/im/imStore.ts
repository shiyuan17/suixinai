...
export class IMStore {
  ...
  getWhatsAppConfig(): WhatsAppOpenClawConfig {
    const stored = this.getConfigValue<WhatsAppOpenClawConfig>('whatsapp');
    return { ...DEFAULT_IM_CONFIG.whatsapp, ...stored };
  }

  setWhatsAppConfig(config: Partial<WhatsAppOpenClawConfig>): void {
  ...
  isConfigured(): boolean {
    ...
    const hasWhatsApp = Boolean(config.whatsapp?.enabled && config.whatsapp?.webhookSecret);
