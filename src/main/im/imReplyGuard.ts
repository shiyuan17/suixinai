...
export const UNSCHEDULED_REMINDER_FAILURE_REPLY = '这次没有真正创建自动化任务，所以不会自动提醒。请重试。';
export const FAILED_REMINDER_FAILURE_REPLY = '自动化任务创建失败，所以不会自动提醒。请重试。';
...
const REMINDER_COMMITMENT_PATTERNS = [
  ...
  /\b(?:created|set up|scheduled)\s+(?:an?\s+)?(?:automation|scheduled)\s+tasks?\b/i,
  /\b(?:automation|scheduled)\s+tasks?\s+created\s+successfully\b/i,
  /(?:我会|我来|稍后|到时间后我会|届时我会|之后我会).{0,24}(?:提醒你|提醒您|通知你|通知您|叫你|叫您)/u,
  /(?:\d+\s*(?:秒|秒钟|分钟|小时|天)后|明天|后天|今晚|稍后).{0,16}(?:会)?(?:提醒你|提醒您|通知你|通知您|叫你|叫您)/u,
  /(?:已|已经).{0,12}(?:为你|帮你|替你)?(?:设置|创建|添加|安排|做好).{0,18}(?:提醒|定时任务|自动化任务|闹钟)/u,
  /(?:定时任务|自动化任务)创建成功/u,
  /到时间后我会(?:自动)?提醒(?:你|您)/u,
];

const REMINDER_NEGATION_PATTERNS = [
  /(?:无法|不能|没法|未能|没有|并未).{0,12}(?:设置|创建|添加|安排).{0,18}(?:提醒|定时任务|自动化任务|闹钟)/u,
  /(?:这次|当前|本次).{0,12}(?:没有真正创建|未真正创建).{0,18}(?:提醒|定时任务|自动化任务)/u,
  /不会自动提醒/u,
  /did not schedule a reminder/i,
  /did not (?:actually )?(?:create|schedule|set up) (?:an? )?(?:automation|scheduled) tasks?/i,
  /failed to schedule/i,
  /failed to (?:create|schedule|set up) (?:an? )?(?:automation|scheduled) tasks?/i,
];
