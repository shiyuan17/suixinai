...
export class IMCoworkHandler extends EventEmitter {
  ...
  private async getOrCreateCoworkSession(
    ...
    if (existing) {
      ...
      if (!session) {
        ...
        this.refreshSessionTitleIfNeeded(existing.coworkSessionId, session.title, platform, imConversationId, senderId, message);
  ...
  private refreshSessionTitleIfNeeded(
    sessionId: string,
    currentTitle: string,
    platform: Platform,
    imConversationId: string,
    ...
    if (!/^IM-whatsapp-\d+$/.test(currentTitle)) return;

    const nextTitle = this.buildSessionTitle(platform, imConversationId, senderId, message);
    if (nextTitle === currentTitle) return;

    this.coworkStore.updateSession(sessionId, { title: nextTitle }, { touchUpdatedAt: false });
  }
   ...
   *
   * WhatsApp direct conversations use the customer display name when available.
  ...
  private buildSessionTitle(
    ...
    if (platform === 'whatsapp') {
      const peerLabel = this.sanitizeSessionTitleLabel(
        message?.groupName || message?.senderName || senderId || _imConversationId
      );
      return `IM-whatsapp-${peerLabel}`;
    }
  ...
  private sanitizeSessionTitleLabel(value: string | undefined): string {
  ...
  private async handleDirectScheduledTaskRequest(
    ...
    try {
      ...
      const reply = `自动化任务创建失败：${errorMessage}`;
      this.coworkStore.addMessage(sessionId, {
        type: 'assistant',
        content: reply,
        metadata: {},
