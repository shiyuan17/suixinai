...
test('creates WhatsApp cowork sessions with customer name in title', async () => {
  const runtime = new FakeRuntime();
  const coworkStore = new FakeCoworkStore();
  const imStore = new FakeIMStore();
  ...
  const responsePromise = handler.processMessage(createMessage({
    platform: 'whatsapp',
    conversationId: '8619902817313@c.us',
    senderId: 'gr h',
    senderName: 'gr h',
    groupName: 'gr h',
    content: 'Hi',
  ...
  await waitForCondition(() => runtime.startCalls.length === 1);
  ...
  expect(coworkStore.sessions.get('session-1')?.title).toBe('IM-whatsapp-gr h');
  handler.destroy();
});
