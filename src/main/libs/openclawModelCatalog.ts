...
const toStringOnlyEnv = (env: NodeJS.ProcessEnv): NodeJS.ProcessEnv => {
...
const indexProviderCatalogBuilders = (
  ...
  for (const [exportName, exported] of Object.entries(providerCatalogModule)) {
    ...
    try {
      const result = exported(toStringOnlyEnv(process.env)) as unknown;
      if (isPromiseLike(result)) {
        void result.then((): void => undefined, (): void => undefined);
        continue;
      }
      providerConfig = result as OpenClawCatalogProvider;
    } catch {
      continue;
    }
